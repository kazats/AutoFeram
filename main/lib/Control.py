...
def control_temperature(
    ...
    os.makedirs(Path.cwd() / 'dipoRavg') # sp.call("mkdir dipoRavg", shell=True)
    os.makedirs(Path.cwd() / 'coords') # sp.call("mkdir coords", shell=True)
    ...
    for temperature in range(Ti, Tf, dT):
        ...
        with open(avg_file) as inf,\
            open(thermo_file) as outf:
            outf.write(inf.read())
