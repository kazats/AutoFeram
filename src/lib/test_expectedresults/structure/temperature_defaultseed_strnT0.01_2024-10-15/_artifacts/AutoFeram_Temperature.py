...
if __name__ == "__main__":
    runner = Runner(
        ...
        output_dir  = project_root() / 'output' / 'verification' / f'temperature_defaultseed_strnT0.01_{timestamp()}',
    )

    config = temp_config(
        material = BTO,
        temp_range = TempRange(initial = 380, final = 50, delta = -200),
        setup = [
            General(
                L            = Int3(4,4,4), #(36, 36, 36),
                n_thermalize = 4, # 0000
                n_average    = 2, # 0000
                n_coord_freq = 6, # 0000
                bulk_or_film = Structure.Strn_T
            ),
            # EFieldDynamic(
            ...
            RandomSeed(
                seed = Int2(123456789, 987654321) # default: '123456789 987654321'
            )
        ]
    )
