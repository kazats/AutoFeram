...
def CopyInitialCoord(initial_coord: Path, output_path: Path):
    return Copy(FileIn(initial_coord), FileOut(output_path))
...
if __name__ == "__main__":
    ...
    copy_initialcoord = CopyInitialCoord(initial_coord, f'{runner.output_dir}/1_preNPT/{runner.sim_name}.restart')

    exit_from_result(run(runner, config, add_pre = copy_initialcoord))
